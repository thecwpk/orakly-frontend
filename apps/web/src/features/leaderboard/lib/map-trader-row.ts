...
export function mapLeaderboardRow(row: TraderLeaderboardRowDto, rank: number): Trader {
  ...
  return {
    ...
    roiPct,
    trades: row.tradeCount ?? row.resolvedMarkets,
    tradeCount: row.tradeCount ?? row.resolvedMarkets,
    bestTradeUsd: row.bestTradeUsd ?? 0,
    marketsTraded: row.marketsTraded ?? 0,
    delta24h: 0,
    streak: 0,
