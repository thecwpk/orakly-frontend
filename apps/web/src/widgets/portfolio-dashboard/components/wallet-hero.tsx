      ...
      <div className="relative">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="grid grid-cols-2 gap-3 sm:max-w-md sm:flex-1">
            <div className="rounded-xl bg-black/35 px-4 py-3 ring-1 ring-white/6">
              <p className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">Available</p>
              <p className="mt-1 font-mono text-lg text-emerald-200/95">{formatCompactUsd(availableUsd)}</p>
            </div>
            <div className="rounded-xl bg-black/35 px-4 py-3 ring-1 ring-white/6">
              <p className="text-[10px] font-bold uppercase tracking-wide text-zinc-500">Locked</p>
              <p
                className={cn(
                  "mt-1 font-mono text-lg",
                  lockedUsd > 0 ? "text-amber-200/90" : "text-zinc-500",
                )}
              >
              ...
              {onChain.balances.map((b) => (
                <div
                  ...
                  className={cn(
                    ...
                    b.isNative ? "bg-cyan-500/10" : "bg-white/[0.04]",
                  )}
                >
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-zinc-500">
                    {b.symbol}
                  </p>
                  <p className="mt-0.5 font-mono text-[13px] text-zinc-100">{b.formattedBalance}</p>
                </div>
              ))}
            </div>
          </div>
