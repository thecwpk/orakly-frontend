...
/** GET /api/v1/profile/:address — public trader profile snapshot. */
export async function GET(_req: Request, ctx: RouteCtx) {
  try {
    ...
    const data = await getTraderProfileByAddress(address);
    if (!data) {
      return NextResponse.json(
        err(API_ERROR_CODES.VALIDATION, "Invalid wallet address"),
        { status: 400 },
    ...
    return NextResponse.json(err("PROFILE_UNAVAILABLE", message), { status: 503 });
  }
}
